...
func check_hit() -> void:
	...
	for i in range(num_rays):
		...
		if result and "collider" in result:
			...
			if collider.get_parent().is_in_group("trees"):
				if not hit_trees.has(collider.get_parent()):  # 去重
					hit_trees.append(collider.get_parent())
			elif collider.get_parent().is_in_group("animals"):
				if not hit_animals.has(collider.get_parent()):  # 去重
					hit_animals.append(collider.get_parent())
	...
	# 对所有在弧形范围内的动物造成伤害
	for animal in hit_animals:
		if animal.has_method("take_damage"):
			animal.take_damage(CHOP_CONFIG.damage)
			print("命中动物！造成", CHOP_CONFIG.damage, "点伤害")

	if not hit_animals.is_empty():
		print("攻击命中" + str(hit_animals.size()) + "个目标！")
